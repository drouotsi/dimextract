...
func doTest(t *testing.T, testCases []TestCase) {
    for _, tc := range testCases {
        ...
        if err != nil {
            t.Errorf("Eeeee %s", err)
        }
        if !tc.equal(&d) {
            t.Errorf("Didnt match expected  %s, got : %s", tc.str(), d.str())
            t.Fail()
        }
    }
}
...
func NoTestExtract(t *testing.T) {
    testCases := []TestCase{
        ...
        {"On y joint un plat en faïence blanche. 25-33 cm", 33, 23, 0},
        ...
        {"Hauteur assise : 43 cm Long.: 110 cm On y joint une galette en tissu destinée à recouvrir le siège.", 110, 43, 0},
        ...
        {"bakélite et métal. Circa 1960. Dans son étui. Long.: 27 cm", 27, 27, 0},
        ...
        {"Paire de bougeoirs en bronze, modèle Rocaille. Montés en lampe, avec leurs abat-jours. Hauteur totale : 28 cm", 26, 0, 0},
        {"Lot de deux cadres en bois et velours, dont un avec application de laiton. 28 x 22 et 35 x 25 cm", 47, 35, 0},
        {"Circa 1960. 19 x 26 x 7 cm TBE.", 19, 16, 7},
